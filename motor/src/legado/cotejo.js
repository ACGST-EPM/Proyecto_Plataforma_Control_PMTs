 ...
 *
 * @param {{filas:Array<Array<string>>, resumen:object}} replica  salida de ejecutarLegado()
 * @param {{relaciones:Array, registros:Array}} motorLegado       salida de analizar(..., PERFIL_LEGADO)
...
export function cotejarFidelidad(replica, motorLegado) {
  ...
  const completo = soloReplica.length === 0 && soloMotor.length === 0 && trazadosCoinciden;
...
export function resumirCotejo(c) {
  if (c.completo) {
    return `Se compararon ${c.coincidentes} alertas una a una y coinciden todas: ` +
      `mismo par de contratos, mismos frentes, misma categoría y mismo periodo. ` +
      `Ninguna falta y ninguna sobra. También coincide el número de trazados leídos (${c.trazadosReplica}).`;
  }
  const partes = [];
  ...
  if (!c.trazadosCoinciden) partes.push(`Además, el número de trazados leídos no cuadra: ${c.trazadosReplica} frente a ${c.trazadosMotor}.`);
  partes.push('Por eso NO se puede afirmar que la reproducción sea completa.');
  return partes.join(' ');
}
